...
class State {
  ...
  */
  act(self) {
    if (!self.determineMetabolicEntitiesSets()) {
      // Specify file with information about application's default state.
      // Define path.
      //var path = "state_default.json";
      //var file = General.accessFileByPath(path);
      // Submit file to model.
      //Action.submitFile(file, self.model);
    ...
    if (!this.determineMetabolicEntitiesSets() && this.determineFile()) {
      // Load from file a default persistent state of the application.
      // The intent is for this action to be temporary during development.
