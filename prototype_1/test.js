...
/**
 * Tests function collectProximalNodes().
 * @returns {boolean} Whether or not the function meets expectation.
 */
function testCollectProximalNodes() {
    ...
    var returnValues = collectProximalNodes(
        network.collection(), ["b", "c", "d", "e", "f"], [], true, true, network
    );
    // Extract identifiers for nodes in collection.
    var collectionNodeIdentifiers = returnValues.collection
        .nodes()
        .map(function (node) {
            return node.id();
        });
...
/**
 * Tests function traverseBreadthByDepth().
 * @returns {boolean} Whether or not the function meets expectation.
 */
function testTraverseBreadthByDepth() {
    var network = cytoscape({
        container: document.getElementById("exploration"),
        elements: {
            edges: [
    ...
    var collection = traverseBreadthByDepth(
        network.collection(), ["a"], true, true, 0, 2, network
    );
    // Extract identifiers for nodes in collection.
